...
{
    if(!isconnecting||msg_queue.size()==0)
        return "";
...
void internet::push_msg(QString res)
{
...
void internet::msg_handle(QString res)
{
    for(int i=0;i<res.size();i++)
    {
        if(res[i]=='/'&&(i+1<res.size()))
        {
            ...
            {
                case 'N':i=msg_split(i+2,res)-1;break;
                case 'S':i=msg_split(i+2,res)-1;break;
                case 'I':i=msg_split(i+2,res)-1;break;
                case 'F':i=msg_split(i+2,res)-1;break;
                case 'Z':i=msg_split(i+2,res)-1;break;
                default:i=msg_split(i+2,res)-1;break;
            }
        }
        else
        {
...
int internet::msg_split(int index,QString msg)
{
    if(index==msg.size())
        return index;
    QString str;
